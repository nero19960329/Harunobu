...
inline void check_any_subnode(
    ...
    HARUNOBU_CHECK(
        has_any_subnode,
        "'{}' node does not have any of subnode '{}'!", node->name(), subnode_names_str);
}
...
sptr<Camera> MitsubaReader::load_camera(rapidxml::xml_node<> *camera_node) {
    ...
    if (trans_mat_node != nullptr) {
        HARUNOBU_DEBUG("Traversing node {}-{}", trans_node->name(), trans_mat_node->name());
        ...
        camera->up = vec3(vec4(0.0, 1.0, 0.0, 0.0) * trans_mat);    // direction does not need to be converted
    } else if (trans_lookat_node != nullptr) {
        HARUNOBU_DEBUG("Traversing node {}-{}", trans_node->name(), trans_lookat_node->name());
