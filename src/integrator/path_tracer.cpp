...
vec3 PathTracer::integrate_ray(const Ray &ray) {
    ...
    for (const auto &light : scene->lights->prims) {
        ...
        auto intersect_light = scene->objects->ray_intersect(light_test_ray, is_intersect_light);
        if (!is_intersect_light || intersect_light->prim != light.get()) {
            // test failed, shadow
            continue;
        }

        real cos_theta_i = std::max(intersect->normal_dot(wi), static_cast<real>(0));
        real cos_theta_o = std::max(glm::dot(intersect_light->normal, -wi), static_cast<real>(0));
